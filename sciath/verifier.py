...
class Verifier(object):  # pylint: disable=bad-option-value,too-few-public-methods,useless-object-inheritance
    ...
    def execute(self, output_path, exec_path=None):
        ...
        raise NotImplementedError(
            "Verifier implementations must override execute()")
...
class ExitCodeVerifier(Verifier):
    ...
    def execute(self, output_path, exec_path=None):
        ...
        exit_code_file = os.path.join(output_path,
                                      self.test.job.exitcode_filename)
        if not os.path.isfile(exit_code_file):
            report.append("[ReturnCodeDiff] File (" + exit_code_file +
                          ") not found")
            passing = False
            info = 'file not found'
        ...
        if exit_codes != self.exit_codes_success:
            report.append("[ExitCodeDiff] Expected exit code(s): " +
                          str(self.exit_codes_success))
            report.append("[ExitCodeDiff] Output exit code(s)  : " +
                          str(exit_codes))
            passing = False
        else:
            passing = True
...
class ComparisonVerifier(Verifier):
    ...
    def execute(self, output_path=None, exec_path=None):
        ...
        if not os.path.isfile(self.expected_file):
            ...
            report.append('[Comparison] Expected file missing: %s' %
                          self.expected_file)
            return passing, info, report
    ...
    def _compare_files(self, from_file, to_file):  # pylint: disable=no-self-use
        ...
        for line in difflib.unified_diff(lines_from,
                                         lines_to,
                                         fromfile=from_file,
                                         tofile=to_file):
    ...
    def _from_file(self, output_path=None, exec_path=None):
        ...
        if self.comparison_file:
            if not exec_path:
                raise Exception(
                    "exec_path must be provided, when a comparison file is specified"
                )
            path = os.path.join(exec_path, self.comparison_file)
        else:
            if not output_path:
                raise Exception(
                    "output_path must be provided, when comparing to an output file"
                )
